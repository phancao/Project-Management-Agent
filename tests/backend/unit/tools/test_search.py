...
class TestGetWebSearchTool:
    @patch("backend.tools.search.SELECTED_SEARCH_ENGINE", SearchEngine.TAVILY.value)
    ...
    @patch("backend.tools.search.SELECTED_SEARCH_ENGINE", SearchEngine.DUCKDUCKGO.value)
    def test_get_web_search_tool_duckduckgo(self):
        tool = get_web_search_tool(max_search_results=3)
        assert tool.name == "web_search"
        assert tool.max_results == 3

    @patch("backend.tools.search.SELECTED_SEARCH_ENGINE", SearchEngine.BRAVE_SEARCH.value)
    ...
    @patch("backend.tools.search.SELECTED_SEARCH_ENGINE", SearchEngine.ARXIV.value)
    ...
    @patch("backend.tools.search.SELECTED_SEARCH_ENGINE", "unsupported_engine")
    ...
    @patch("backend.tools.search.SELECTED_SEARCH_ENGINE", SearchEngine.TAVILY.value)
    @patch("backend.tools.search.load_yaml_config")
